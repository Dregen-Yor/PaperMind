...
/** 偏移连通容差（字符）（#7）：真实碎片 offset 无缝衔接（gap 为 0），容差只覆盖 1-2 字符的空白-only 间隙。 */
 ...
 * 与偏移连通（next.startOffset <= 簇内 maxEnd + 2），每簇并作一条。
 */
export function planFragmentMerge(rows: FragmentRow[]): FragmentMergePlan {
  ...
  for (const list of groups.values()) {
    ...
    for (const row of sorted.slice(1)) {
      ...
      const offsetConnected = row.startOffset <= clusterMaxEnd + OFFSET_GAP_TOLERANCE
      if (timeConnected && offsetConnected) {
        cluster.push(row)
