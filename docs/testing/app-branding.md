...
> **2026-09-22 后续变更（图标母版已更新）**：`assets/papermind-icon.svg` 已更换为新设计（米白圆角底板 + 胡桃棕 P 字标，画板自带边距 87.7%）。全部平台图标已重新生成，macOS 缩放由 `0.82` 调整为 `0.92`（主体占画布 **80.86%**，对照 Apple 网格 80.47%；若沿用 0.82 会缩到 72.07%，比相邻图标明显偏小）。因此：本文档第二、三节里的图标哈希（`63ee71b7…`）与 82.03% 主体比例属于**更换前**的图标集与那次 DMG 构建，本次未重新打包安装包；开发副本会在下次 `npm run dev` 时按新图标自动重建（指纹含图标字节，已实测重建后副本 ICNS 与新母版产物同哈希、`codesign --verify --deep --strict` 通过）。重复运行 `npm run icons:generate` 结果逐字节一致。
